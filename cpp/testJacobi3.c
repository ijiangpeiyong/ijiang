//pgcc -c99 -acc -Minfo testJacobi2.c -o main_acc.exe
...
int main()
{
    ...
    float u0[(ROW + 1) * COLp1];
    float u1[(ROW + 1) * COLp1];
...
#pragma acc loop independent
            for (ix = 1; ix < ROW; ix++)
            {
                for (jy = 1; jy < COL; jy++)
                {
                    u0[ix*COLp1 + jy] = c2 *
                        (
                            hy2 * (u1[(ix - 1) * COLp1 + jy] + u1[(ix + 1) * COLp1 + jy]) +
                            hx2 * (u1[ix * COLp1 + (jy - 1)] + u1[ix * COLp1 + (jy + 1)]) +
                            c1
                        );
                    //temp = fabs(u0[ix * COLp1 + jy] - u1[ix * COLp1 + jy]);
                    //uerr = max(temp, uerr);
